...
def get_pipeline_stats():
    ...
    stats = {
        ...
        "Count": [
            ...
            get_file_count(CHUNKS_DIR, "*.md") - get_file_count(CHUNKS_DIR, "*.md", processed_only=True),
            get_file_count(OPTIMIZED_DIR, "*.md") - get_file_count(OPTIMIZED_DIR, "*.md", processed_only=True),
            get_file_count(OPTIMIZED_DIR, "*.md", processed_only=True) + get_file_count(CHUNKS_DIR, "*.md", processed_only=True),
...
def display_progress():
    ...
    if stats["Count"][0] > 0:  # If there are PDF files
        ...
        chunk_progress = min(100, current_chunks / total_expected_chunks * 100 if total_expected_chunks > 0 else 0)
        
        audio_progress = stats["Count"][5] / current_chunks * 100 if current_chunks > 0 else 0
        chapter_progress = stats["Count"][6] / total_chapters * 100 if total_chapters > 0 else 0
...
def main():
    ...
    parser.add_argument(
        "--log", 
        action="store_true", 
        help="Log current progress to CSV file"
    )
    
    parser.add_argument(
        "--plot", 
        action="store_true", 
        help="Generate a plot of progress over time"
    )
