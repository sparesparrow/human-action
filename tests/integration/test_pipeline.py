...
class TestPipeline:
    ...
    def test_full_pipeline(self, mock_separator, mock_concatenator, 
                          mock_audio_generator, mock_chunker,
                          mock_pdf_processor, test_config, create_test_pdf):
        """
        ...
        for chapter_idx, _ in enumerate(markdown_files, 1):
            for chunk_idx in range(2):  # 2 chunks per chapter
                chunk_file = test_config.markdown_chunks_dir / f"chapter_{chapter_idx:02d}{chr(97+chunk_idx)}.md"
        ...
        for chunk_idx, _ in enumerate(chunk_files):
            for para_idx in range(2):  # 2 paragraphs per chunk
                para_file = test_config.markdown_paragraphs_dir / f"para_{chunk_idx}_{para_idx}.md"
        ...
        pdf_extractor_mock.PDFProcessor.return_value.process.return_value = markdown_files
        ...
        with patch.dict('sys.modules', {
            ...
            'espeak_audio_chunk_generator': MagicMock(EspeakAudioChunkGenerator=audio_gen_mock),
            'audio_concatenator': MagicMock(AudioConcatenator=audio_concat_mock),
            'scripts.separate_paragraphs': MagicMock(ParagraphSeparator=para_sep_mock)
        }):
            # Run the pipeline with explicit arguments
            result = pipeline.process(
                test_pdf_path,
                steps=['pdf_extraction', 'chunk_splitting', 'paragraph_separation', 
                       'audio_generation', 'audio_concatenation'],
    ...
    def test_pipeline_state_tracking(self, test_config, create_test_pdf):
        ...
        assert 'pdf_extraction' in state_data['completed_steps'], "PDF extraction step not marked as completed"
        assert state_data['input_file'] == str(test_pdf_path), "Input file not correctly stored in state"
        ...
        with patch.dict('sys.modules', {
            'pdf_extractor': MagicMock(PDFProcessor=lambda: pdf_processor_mock),
            'chunker_splitter': MagicMock(MarkdownChunker=lambda *args, **kwargs: chunker_mock)
        }):
        ...
        assert 'pdf_extraction' in state_data['completed_steps'], "PDF extraction step missing from state"
        assert 'chunk_splitting' in state_data['completed_steps'], "Chunk splitting step missing from state"
    ...
    def test_pipeline_with_espeak(self, test_config, create_test_pdf):
        ...
        state = {
            ...
            "completed_steps": {
                "pdf_extraction": {
                    "output_files": [str(test_config.markdown_chapters_dir / "chapter_01.md")]
                },
                "chunk_splitting": {
                    "output_files": [str(path) for path in chunk_files]
                },
                "paragraph_separation": {
                    "output_files": [str(test_config.markdown_paragraphs_dir / "para_01.md")]
                }
            }
        }
        ...
        audio_concatenator_mock.process.return_value = config.audio_book_dir / "audiobook.mp3"
        ...
        # Patch modules
        with patch.dict('sys.modules', {
            'audio_concatenator': MagicMock(AudioConcatenator=lambda *args, **kwargs: audio_concatenator_mock)
        }):
            # Run just the audio concatenation step
            result = pipeline.process(
                test_pdf_path,
                steps=['audio_concatenation'],
                state_file=state_file
            )
        
        # Check results
        assert result is not None, "Pipeline returned None"
        assert "audiobook.mp3" in str(result), "Pipeline did not return the expected audiobook file"
        
        # Check that the state has been updated
        with open(state_file, 'r') as f:
            state_data = json.load(f)
        
        assert 'audio_concatenation' in state_data['completed_steps'], "Audio concatenation step not in completed steps" 