...
class TestParagraphSeparator:
    ...
    def test_separate_paragraphs(self, test_config):
        ...
        assert "This is the first paragraph" in paragraph_texts[1], "First paragraph not preserved"
        assert "This is the second paragraph" in paragraph_texts[2], "Second paragraph not preserved"
        assert "## Section Heading" in paragraph_texts[3], "Section heading not preserved"
        assert "This is the third paragraph" in paragraph_texts[4], "Third paragraph not preserved"
    ...
    def test_process_directory(self, test_config):
        ...
        assert len(all_output_files) >= 8, f"Expected at least 8 output files, got {len(all_output_files)}"
        ...
        for i in range(2):
            ...
            chapter_files = [stem for stem in file_stems if stem.startswith(chapter_prefix)]
