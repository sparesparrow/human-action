...
class TestPDFExtractor:
    ...
    def test_extract_text_from_pdf(self, mock_pdf_reader, test_config):
        ...
        page2 = MagicMock()
        page2.extract_text.return_value = "This is page 2 with some Czech characters: ěščřžýáíé"
        ...
        with patch.object(pdf_extractor.PDFProcessor, 'extract_text_from_pdf', mock_extract_text):
        ...
        assert "Czech characters: ěščřžýáíé" in result_text, "Czech characters not preserved"
    ...
    def test_process_pdf_file(self, mock_pdf_reader, test_config):
        ...
        for i, file_path in enumerate(sorted(output_files), 1):
            ...
            assert f"chapter_{i:02d}.md" in str(file_path), f"Incorrect file naming"
            
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                assert len(content) > 0, "Chapter file is empty"
                assert f"Chapter {i}" in content, "Chapter title not found" 