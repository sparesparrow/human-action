...
class TestAudioConcatenator:
    ...
    def test_concatenate_audio_files(self, mock_subprocess_run, test_config):
        ...
        result_file = concatenator.concatenate_audio_files(
            audio_files=audio_files, output_file=output_file
        )
    ...
    def test_create_audio_book(self, mock_subprocess_run, test_config):
        ...
        for i in range(3):
            file_path = (
                input_dir / f"chapter_01{chr(97+i)}.mp3"
            )  # chapter_01a, chapter_01b, chapter_01c
            with open(file_path, "wb") as f:
                f.write(b"mock audio data")

        # Create the concatenator
        concatenator = audio_concatenator.AudioConcatenator(
            input_dir=input_dir, output_dir=test_config.audio_book_dir
        )

        # Mock concatenate_audio_files to return a predefined output file
        expected_output = test_config.audio_book_dir / "audiobook.mp3"
        with patch.object(
            concatenator, "concatenate_audio_files", return_value=expected_output
        ) as mock_concatenate:
        ...
        assert all(
            file.parent == input_dir for file in audio_files
        ), "Audio files not from input directory"
        ...
        assert sorted(actual_filenames) == sorted(
            expected_filenames
        ), "Audio files not in the expected order"
