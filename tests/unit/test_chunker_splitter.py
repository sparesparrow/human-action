...
class TestChunkerSplitter:
    ...
    def test_split_into_chunks(self, test_config, setup_test_files):
        ...
        for file_path in output_files:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                assert len(content) <= 1200, f"Chunk size exceeds max_chars significantly: {len(content)}"
        ...
        for i, file_path in enumerate(sorted(output_files)):
            ...
            assert f"chapter_01{expected_suffix}" in str(file_path), f"Wrong file naming pattern for chunk {i}"
    ...
    def test_split_with_too_small_max_chars(self, test_config, setup_test_files):
        ...
        assert len(output_files) > 0, "No output files were generated with small max_chars"
