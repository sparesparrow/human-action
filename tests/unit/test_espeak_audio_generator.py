...
class EspeakAudioChunkGenerator:
    ...
    def synthesize_chunk(self, input_file):
        ...
        cmd = ["espeak", f"--voice={self.language}", "--stdout", text]
...
class TestEspeakAudioGenerator:
    ...
    def test_synthesize_chunk(self, mock_subprocess_run, test_config, setup_test_files):
        ...
        assert (
            output_file.stem == "test_chunk"
        ), "Output filename doesn't match input stem"
    ...
    @patch("subprocess.run")
    def test_process_all_chunks(
        self, mock_subprocess_run, test_config, setup_test_files
    ):
        ...
        for i, output_file in enumerate(sorted(output_files)):
            assert (
                output_file.parent == output_dir
            ), f"Output file {i} in wrong directory"
            assert (
                output_file.stem == f"chunk_{i}"
            ), f"Output filename {i} doesn't match input stem"
