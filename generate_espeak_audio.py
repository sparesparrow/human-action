...
try:
    ...
    print("Warning: espeakng module not found. Will use command-line espeak-ng instead.")
...
def process_markdown_file(
    ...
    try:
        # Create Path objects
        file_path = Path(file_path) # i.e. data/4-markdown-chunks-optimized/chapter_30a-OPTIMIZED.md
        ...
        if not text_content.strip():
            logger.warning(f"File {file_path.name} is empty or contains only whitespace")
        ...
        if ESPEAK_AVAILABLE:
            try:
                ...
                ffmpeg_cmd = [
                    'ffmpeg', '-y', '-i', str(wav_output_file), 
                    '-codec:a', 'libmp3lame', '-qscale:a', FFMPEG_QUALITY,
                    str(mp3_output_file)
                ]
                with subprocess.Popen(
                    ...
                    if proc.returncode != 0:
                        raise subprocess.CalledProcessError(proc.returncode, ffmpeg_cmd, stderr)
        ...
        try:
            ...
            espeak_cmd = [
                ...
                '-w', str(wav_output_file)
            ]
            subprocess.run(espeak_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def find_remaining_files(input_dir, progress):
    ...
    for file in all_files:
        ...
        if (file not in processed_files and 
            not file_path.name.startswith("AUDIO_GENERATED-") and
            not file_path.name.startswith("ESPEAK_AUDIO-")):
...
def main():
    ...
    if args.ignore_progress:
        ...
        progress = {"processed_files": [], "failed_files": [], "total_processed": 0, "total_failed": 0, "last_run": None}
    else:
    ...
    logger.info(f"Total processed: {progress['total_processed']}, Total failed: {progress['total_failed']}")
