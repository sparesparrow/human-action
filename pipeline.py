...
class Pipeline:
        ...
        for step in steps_to_run:
            if step in processor_map:
                ...
                if step == 'pdf_extraction':
                    ...
                    processor = processor_class(
                        self.config.get_path(step + '_input') or self.config.markdown_chunks_dir,
                        self.config.get_path(step + '_output') or self.config.audio_chunks_dir
                    )
                ...
                self.state['completed_steps'][step] = {
                    'output_files': [str(f) for f in result] if isinstance(result, list) else [str(result)]
                }
                self._save_state()
    ...
    def get_audio_generator(self):
        ...
        return EspeakAudioChunkGenerator(
            self.config.markdown_chunks_dir,
            self.config.audio_chunks_dir
        )