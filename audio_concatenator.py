...
def get_chapter_number(filename: str) -> Optional[str]:
...
class AudioConcatenator:
    ...
    def process(self, output_filename: str = "audiobook.mp3") -> Path:
        ...
        mp3_files = sorted(
            self.input_dir.glob("*.mp3"), key=lambda p: natural_sort_key(p.name)
        )
...
def concatenate_audio_files(input_files: List[Path], output_file: Path) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Successfully concatenated {len(input_files)} files into {output_file}"
        )
        return True
...
def process_chapter(
    chapter_num: str, files: List[Path], output_dir: Path
) -> Optional[Path]:
    """
...
def process_all_chapters(input_dir: Path, output_dir: Path) -> int:
    ...
    logger.info(
        f"Processed {successful_chapters} out of {len(chapters)} chapters successfully"
    )
...
def main():
    ...
    if args.chapter:
        ...
        mp3_files = sorted(
            input_dir.glob(f"chapter_{args.chapter}*.mp3"),
